...
```{r input data for self organizing maps, message = F, warning = F}
som <- summary %>%
  select(
    stn,
    mean_chl_ap676lh,
...
```{r scaled training data for self organizing map}
...
```{r combine cluster id with data}
...
```{r compute biomass index from z-scores}
index <-  summary %>%
  select(
    stn,
    mean_chl_ap676lh,
