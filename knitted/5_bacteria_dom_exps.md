...
``` r
fig4_data <- p5_prod %>%
  select(exp, trt, biomass, composite_z, days, bge_inclusion, mean_toc, boc) %>%
  rename(toc = mean_toc) %>%
  ...
  left_join(
    .,
    p5_prod %>%
      select(exp, trt, biomass, days, bge_inclusion,  composite_z, sd_toc) %>%
   ...
   bind_rows(
    .,
    p5_prod %>%
      select(exp, trt, biomass, days, bge_inclusion,  composite_z, doc, sd_toc) %>%
  ...
  bind_rows(
    .,
    p5_prod %>%
      select(exp, trt, biomass, days,  bge_inclusion, composite_z, mean_cells_ml, sd_cells_ml) %>%
...
metric_data <- p5_prod %>%
  ...
  mutate(
    var = case_when(
      var == "mean_gr_d" ~ "bold(Specific~growth~rate~(d^-1))",
