...
rates <- apparent_gr %>%
  ...
  mutate(g = ifelse(d != 0, (k - k[d == 0])/d, NA),
...
p_rates <- rates %>% 
  ...
  mutate(rate = case_when(rate == "mu" ~ "bold(Growth~(d^-1))",
...
p_rates_summary <- rates %>% 
  ...
  distinct() %>% 
  pivot_longer(c(ave_mu, ave_g, ave_r), names_to = "rate", values_to = "val") %>% 
  mutate(rate = case_when(rate == "ave_mu" ~ "bold(Growth~(d^-1))",
  ...
  distinct() %>% 
  pivot_longer(c(sd_mu, sd_g, sd_r), names_to = "rate", values_to = "sd") %>% 
  mutate(rate = case_when(rate == "sd_mu" ~ "bold(Growth~(d^-1))",
...
dots <- ggplot(p_rates_summary ,
  ...
  stat_smooth(aes(fill = factor(amend, levels = plot_levels)),method = "gam", formula = y ~ s(x, bs = "tp"), alpha = 0.25, size = 2) +
